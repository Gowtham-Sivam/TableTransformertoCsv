...
async def pytess(cell_pil_img):
    return ' '.join(pytesseract.image_to_data(cell_pil_img, output_type=Output.DICT,
...
class TableExtractionPipeline():
    ...
    def plot_results_detection(self, c1, model, pil_img, prob, boxes, delta_xmin, delta_ymin, delta_xmax, delta_ymax,
        ...
        plt.axis('off')
        c1.pyplot()
        pil_img.save(os.path.join(filename, 'table_detection.jpg'))
    ...
    def generate_structure(self, c2, model, pil_img, prob, boxes, expand_rowcol_bbox_top, expand_rowcol_bbox_bottom,
        ...
        plt.imshow(pil_img)
        pil_img.save(os.path.join(filename, 'table_%i.jpg') % pg_no)
        ax = plt.gca()
        rows = {}
        cols = {}
        idx = 0
        ...
        for p, (xmin, ymin, xmax, ymax) in zip(prob, boxes.tolist()):
            ...
            if (class_text == 'table row') or (class_text == 'table projected row header') or (
                    class_text == 'table column'):
                ax.add_patch(
                    plt.Rectangle((xmin, ymin), xmax - xmin, ymax - ymin, fill=False, color=self.colors[cl.item()],
                                  linewidth=2))
    ...
    def sort_table_featuresv2(self, rows: dict, cols: dict):
        ...
        rows_ = {table_feature: (xmin, ymin, xmax, ymax) for table_feature, (xmin, ymin, xmax, ymax) in
                 sorted(rows.items(), key=lambda tup: tup[1][1])}
        cols_ = {table_feature: (xmin, ymin, xmax, ymax) for table_feature, (xmin, ymin, xmax, ymax) in
                 sorted(cols.items(), key=lambda tup: tup[1][0])}
    ...
    def object_to_cellsv2(self, master_row: dict, cols: dict, expand_rowcol_bbox_top, expand_rowcol_bbox_bottom,
                          padd_left):
    ...
    async def start_process(self, image_path: str, TD_THRESHOLD, TSR_THRESHOLD, padd_top, padd_left, padd_bottom,
                            padd_right, delta_xmin, delta_ymin, delta_xmax, delta_ymax, expand_rowcol_bbox_top,
        ...
        self.plot_results_detection(c1, model, image, probas, bboxes_scaled, delta_xmin, delta_ymin, delta_xmax,
                                    delta_ymax, filename)
        cropped_img_list = self.crop_tables(image, probas, bboxes_scaled, delta_xmin, delta_ymin, delta_xmax,
                                            delta_ymax)
        pg_no = 1
        for unpadded_table in cropped_img_list:
            ...
            rows, cols = self.generate_structure(c2, model, table, probas, bboxes_scaled, expand_rowcol_bbox_top,
                                                 expand_rowcol_bbox_bottom, filename, pg_no)
            ...
            cells_img, max_cols, max_rows = self.object_to_cellsv2(master_row, cols, expand_rowcol_bbox_top,
            ...
            df_tosave = self.create_dataframe(c3, cells_pytess_result, max_cols, max_rows)
            df_tosave.to_csv(os.path.join(filename, 'table_%i.csv') % pg_no, mode='a')
            pg_no = pg_no + 1
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        ...
        img_name = str(sys.argv[1])
        padd_top = int(sys.argv[2])
        padd_left = int(sys.argv[3])
        padd_right = int(sys.argv[4])
        padd_bottom = int(sys.argv[5])
        TD_th = float(sys.argv[6])
        TSR_th = float(sys.argv[7])
    else:
        print(
            "Enter arguments: location padd_top padd_left padd_right padd_bottom Table detection threshold Table structure recognition threshold")
        print("Eg: python tdtsr.py 'c:\desktop\ 1.png' 30 30 30 30 0.4 0.8 ")
    ...
    if img_name is not None:
        asyncio.run(
            te.start_process(img_name, TD_THRESHOLD=TD_th, TSR_THRESHOLD=TSR_th, padd_top=padd_top, padd_left=padd_left,
                             padd_bottom=padd_bottom, padd_right=padd_right, delta_xmin=0, delta_ymin=0, delta_xmax=0,
                             delta_ymax=0, expand_rowcol_bbox_top=0, expand_rowcol_bbox_bottom=0))
